...
-   **罡风 / 天罡** — when 天罡 appears as a proper-noun prefix in technique names, render as `Astral` (e.g., 天罡莽牛劲 → `Astral Wild Bull Force`). When 罡风 appears as the literal manifestation of that energy in combat description, render as `astral wind` (e.g., 罡风涌动 → `astral wind surged`). The two renderings share the "Astral" root so that the manifestation imagery links back to the technique name on the page. Avoid downstream renderings like `gale` for 罡风 in chapters where the same technique's name is also being translated — that breaks the link a reader would otherwise pick up. The Pinyin loan `Heaven-Gang` was tried and dropped because the "gang" syllable visually collides with English "gang" (criminal group); `Astral` carries the celestial-grade meaning of 天罡 without the homonym trap.
...
-   **被 X 支配 / 受 X 影响 / 被 X 笼罩** (literally "ruled by X / affected by X / shrouded by X" for emotional states) → use the natural English emotion idiom: `overcome by`, `gripped by`, `consumed by`, `seized by`, `lost to`. Specifically: 被愤怒支配 → `overcome by rage`, not `ruled by rage`. 被恐惧笼罩 → `gripped by fear`, not `shrouded by fear`. The Chinese passive-with-abstract-noun construction reads as a translation artifact in English; the standard English idiom for emotion-as-overwhelming-force uses `overcome / gripped / seized`.
-   **老子** (vulgar self-aggrandizing "I" — literally "old-man," but functions as a swagger pronoun, not a relational claim) → `I` (with whatever defiant register the line carries) when speaking to a non-relative; the literal `your old man` reads correctly only when the speaker IS the addressee's father. Wang Lang context: 老子和你拼了 → `I'm taking you with me!`, not `your old man's going all in`. Su-Beilou-to-his-son context: `your old man is trying to save your life` works because the literal kinship is true. Different from 老夫 (formal "this old one") and 本X (pompous "this [title]"); don't conflate the renderings.
