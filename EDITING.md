...
-   **罡风 / 天罡** — when 天罡 appears as a proper-noun prefix in technique names, render as `Heaven-Gang` (e.g., 天罡莽牛劲 → `Heaven-Gang Wild Bull Force`). When 罡风 appears as the literal manifestation of that energy in combat description, render as `gang-wind` (e.g., 罡风涌动 → `gang-wind surged`). The two renderings deliberately share the "Gang" root so that the manifestation imagery links back to the technique name on the page. Avoid downstream renderings like `gale` for 罡风 in chapters where the same technique's name is also being translated — that breaks the link a reader would otherwise pick up.
